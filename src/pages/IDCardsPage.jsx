...
function IDCard({ student, section }) {
  ...
  return (
    <div className="id-card" style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: T.radius, padding: 16, textAlign: 'center', boxShadow: T.cardShadow }}>
      {qrSrc ? (
        <img src={qrSrc} alt={`QR code for LRN ${student.lrn}`} width={140} height={140} style={{ display: 'block', margin: '0 auto 10px' }} />
      ) : failed ? (
        <div style={{ width: 140, height: 140, margin: '0 auto 10px', display: 'grid', placeItems: 'center', border: `1px dashed ${T.border}`, borderRadius: 8 }}>
          <span style={{ ...T.num, fontSize: 11, color: T.inkMuted }}>QR unavailable</span>
        </div>
      ) : (
        <div style={{ width: 140, height: 140, margin: '0 auto 10px' }} />
      )}
      <div style={{ fontFamily: T.body, fontWeight: 700, fontSize: 13, color: T.ink }}>{fullName(student)}</div>
      <div style={{ ...T.num, fontSize: 12, color: T.inkMuted, marginTop: 2 }}>{student.lrn}</div>
      <div style={{ fontFamily: T.body, fontSize: 11, color: T.inkMuted, marginTop: 2 }}>{sectionLabel(section)}</div>
    </div>
  );
}
    ...
    <div>
      <style>{`
        @media print {
          ...
          .id-cards-grid { grid-template-columns: repeat(3, 1fr) !important; }
        }
      `}</style>
      ...
      {!section ? (
        ...
        <div className="id-cards-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 16 }}>
          {roster.map((s) => <IDCard key={s.id} student={s} section={section} />)}
        </div>
      )}
    </div>
  );
